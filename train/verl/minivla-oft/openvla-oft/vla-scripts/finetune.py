...
class FinetuneConfig:
    ...
    conf_path: str = "openvla/openvla-7b"             # Path to OpenVLA model (on HuggingFace Hub or stored locally)
    ...
    # revision version
    version: str = "raw"                             # the version number
    # fmt: on
...
def run_forward_pass(
    ...
    # Try using this token to see if it can converge
    ...
    # Compute metrics for continuous action representations (L1 regression | diffusion)
        ...
        if cfg.version =='v1':
            ...
            # print(multi_layer_hidden_states.size())
        else:
            last_hidden_states = output.hidden_states[-1]  # (B, seq_len, D)
            # Get hidden states for text portion of prompt+response (after the vision patches)
        ...
        if use_l1_regression:
            if cfg.version =='v1':
                predicted_actions = action_head.module.predict_action(multi_layer_hidden_states,
                                                proprio=batch["proprio"] if use_proprio else None,
                                                proprio_projector=proprio_projector if use_proprio else None,
                                                phase=cfg.phase)
            else:
            ...
            # aux_loss = torch.nn.HuberLoss()(ground_truth_actions, aux_action)
            ...
            # Predict noise
            if cfg.version =='v1':
                noise_pred = action_head.module.predict_noise(multi_layer_hidden_states,
            ...
            flow_pred = action_head.module.predict_flow(actions_hidden_states)
            # Get flow matching loss
            flow_pred = flow_pred.reshape(flow.shape)
            loss = nn.functional.mse_loss(flow_pred, flow, reduction="mean")
        ...
        should_log_l1_loss = use_l1_regression or (use_diffusion and compute_diffusion_l1) or (use_flow_matching and compute_diffusion_l1)
...
def run_flow_matching_sampling(
    ...
    noise = action_head.module.sample_noise(shape=(batch_size, NUM_ACTIONS_CHUNK, ACTION_DIM),device=device_id)

    dt = -1.0 / action_head.module.num_flow_steps 
    dt = torch.tensor(dt, dtype=torch.bfloat16, device=device_id)

    curr_noisy_actions = noise
    time = torch.tensor(1.0, dtype=torch.bfloat16, device=device_id)
...
def save_training_checkpoint(
    ...
    if cfg.use_lora and cfg.merge_lora_during_training:
        if cfg.use_minivla:
            config = AutoConfig.from_pretrained(cfg.conf_path)
            base_vla = AutoModelForVision2Seq.from_config(config, torch_dtype=torch.bfloat16)  # Create new model with random initialized parameters from config
            ...
            cfg.vla_path, torch_dtype=torch.bfloat16, low_cpu_mem_usage=False, trust_remote_code=False
        )
...
def finetune(cfg: FinetuneConfig) -> None:
    ...
    # if cfg.version == 'v1' or cfg.use_flow_matching:
    ...
    # Modified trust_remote_code=False
    AutoProcessor.register(OpenVLAConfig, PrismaticProcessor)
    ...
    if cfg.version == 'v1':
        if cfg.use_minivla:
            if cfg.phase=='stage2':
                ...
                config = AutoConfig.from_pretrained(cfg.conf_path)
                vla = AutoModelForVision2Seq.from_config(config, torch_dtype=torch.bfloat16).to(device_id)  # Create new model with random initialized parameters from config
    ...
    if cfg.use_flow_matching:
        action_head = init_module(
            ...
            device_id,
            {
                ...
                "hidden_dim": vla.module.llm_dim,
                "action_dim": ACTION_DIM,
                "num_flow_steps": cfg.num_flow_matching_steps
            },
            to_bf16=True,
        )
        noisy_action_projector = init_module(
    ...
    #         optimizer,
    #         T_max=cfg.num_steps_before_decay,  # 退火周期长度
    #         eta_min=0,                        # 最小学习率，一般为0
    #         )
    ...
    with tqdm.tqdm(total=cfg.max_steps, leave=False) as progress:
        ...
        for batch_idx, batch in enumerate(dataloader):
            ...
            compute_diffusion_l1 = (cfg.use_l1_regression and batch_idx % cfg.diffusion_sample_freq == 0) or (cfg.use_diffusion and batch_idx % cfg.diffusion_sample_freq == 0) or (cfg.use_flow_matching and batch_idx % cfg.diffusion_sample_freq == 0)
